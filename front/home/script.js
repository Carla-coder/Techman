...
function renderComentarios(id) {
  ...
  comentarios.forEach((e) => {
      if (e.equipamentoId === id) {
          ...
          title.innerHTML = getPerfilNome(e.perfil) + " - " + formatDate(e.data);
          description.innerHTML = e.comentario;
          card.appendChild(title);
          card.appendChild(description);
          telaComentarios.appendChild(card);
...
// Função auxiliar para obter o nome do perfil com base no perfilId
...
function modalComentario(id) {
  // Verificar se o modal de comentários existe
  const modal = document.getElementById("modalComentarios");
  if (modal) {
    modal.style.display = "flex";  // Mostra o modal
    renderComentarios(id);  // Função para renderizar os comentários

    // Verificar se o botão 'adicionarComentarios' existe antes de atribuir o evento
    const btnAdicionar = document.getElementById("adicionarComentarios");
    if (btnAdicionar) {
      btnAdicionar.setAttribute(
        "onclick",
        `modalNewComentario(${id});this.parentNode.style.display = 'none';`
      );
    } else {
      console.error("Elemento 'adicionarComentarios' não encontrado no DOM.");
    }

    // Verificar se o botão 'fecharModalComentarios' existe antes de atribuir o evento
    const btnFechar = document.getElementById("fecharModalComentarios");
    if (btnFechar) {
      btnFechar.onclick = function () {
        modal.style.display = "none";
      };
    } else {
      console.error("Elemento 'fecharModalComentarios' não encontrado no DOM.");
    }
  } else {
    console.error("Modal 'modalComentarios' não encontrado no DOM.");
  }
}
